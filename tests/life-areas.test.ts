...
import { computeReviewStats, findOpenSlot, findScheduleConflicts } from '../src/features/planner/engine.ts';
import { buildTimelineEvents } from '../src/features/timeline/projector.ts';
import type { LifeOsSnapshot } from '../src/core/domain.ts';

test('parses goal person money budget recurring savings and time logs', () => {
  ...
  const timeLog = parseLifeOsEntity('Time Logs/g.md','g',{lifeos_type:'time_log',title:'개발',date:'2026-09-18',start_time:'09:00',end_time:'10:30',category:'집중'});
  ...
  assert.equal(timeLog?.kind,'timeLog');
});
...
test('review analytics summarize time-use without collapsing life areas into one score', () => {
  const snapshot:LifeOsSnapshot = {
    blocks:[],tasks:[],projects:[],goalActions:[],
    timeLogs:[
      {id:'1',title:'개발',date:'2026-09-18',startTime:'09:00',endTime:'10:30',category:'집중'},
      {id:'2',title:'산책',date:'2026-09-18',startTime:'12:00',endTime:'12:30',category:'재충전'}
    ]
  };
  const stats=computeReviewStats(snapshot,'2026-09-18');
  assert.equal(stats.timeUse.totalMinutes,120);
  assert.deepEqual(stats.timeUse.categories,[{category:'집중',minutes:90},{category:'재충전',minutes:30}]);
});
