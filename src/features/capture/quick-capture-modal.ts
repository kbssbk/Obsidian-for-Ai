...
export class QuickCaptureModal extends Modal {
  private kind:CaptureKind='task'; private title=''; private date=''; private start='09:00'; private end='10:00'; private priority=3; private estimatedMinutes=30; private amount=0; private moneyKind:'expense'|'income'|'savings'='expense'; private category='생활'; private relationship='기타'; private phone=''; private currentAmount=0;
  ...
  private renderForm():void{
    ...
    if(this.kind==='budget'){
      this.categoryField(contentEl,'분류','생활, 교통 등');this.amountField(contentEl,'월 예산',(value)=>{this.amount=value;});new Setting(contentEl).setName('월').addText((text)=>text.setPlaceholder('YYYY-MM').setValue(this.date).onChange((value)=>{this.date=value.trim();}));
    }
    if(this.kind==='recurring'){
      this.amountField(contentEl,'금액',(value)=>{this.amount=value;});this.dateField(contentEl,'다음 결제일');
    }
    if(this.kind==='savings'){
      this.amountField(contentEl,'목표 금액',(value)=>{this.amount=value;});this.amountField(contentEl,'현재 금액',(value)=>{this.currentAmount=value;});this.dateField(contentEl,'목표일');
    }
  ...
  private async submit():Promise<void>{
    ...
    if(this.kind==='budget')path=await this.repository.createLifeNote('budget',this.title,{month:this.date,category:this.category,limit_amount:this.amount});
