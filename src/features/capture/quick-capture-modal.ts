...
type CaptureKind = 'task' | 'block' | 'goal' | 'person' | 'money';
...
export class QuickCaptureModal extends Modal {
  ...
  private renderForm():void {
    ...
    contentEl.createEl('h2',{text:'빠른 추가'});
    contentEl.createEl('p',{text:'어느 화면에 있든 일정·업무·목표·사람·돈을 바로 기록합니다.'});
    new Setting(contentEl).setName('종류').addDropdown((dropdown)=>dropdown
      .addOptions({task:'업무',block:'일정',goal:'목표',person:'사람',money:'돈'})
      .setValue(this.kind).onChange((value)=>{this.kind=value as CaptureKind; this.title=''; this.renderForm();}));
  ...
  private placeholder():string {
    return this.kind==='task'?'해야 할 일':this.kind==='block'?'일정 이름':this.kind==='goal'?'목표 이름':this.kind==='person'?'이름':'예: 장보기';
  }

  private async submit():Promise<void> {
    if(!this.title){new Notice('제목을 입력하세요.');return;}
    let path='';
