...
export class DashboardView extends ItemView {
  ...
  async render(): Promise<void> {
    ...
    root.createEl('h1', { text:'오늘의 생활 로비' });
    root.createEl('p', { text:`${today} · 가장 중요한 다음 행동부터 시작합니다.` });
    ...
    const dashboard = buildDashboard(snapshot, today);
    ...
    const focus = root.createDiv({ cls:'life-os-card life-os-focus' });
    focus.createEl('h2', { text:'지금 할 한 가지' });
    if (plan.focus) {
      focus.createEl('strong', { text:plan.focus.title });
      focus.createEl('p', { text:`우선순위 ${plan.focus.priority ?? 3} · ${plan.focus.estimatedMinutes ?? 30}분${plan.focus.due ? ` · ${plan.focus.due}` : ''}` });
      if (plan.focus.path) focus.addEventListener('click', () => void this.app.workspace.openLinkText(plan.focus?.path ?? '', '', false));
    } else focus.createEl('p', { text:'새로 급한 일은 없습니다.' });
    ...
    for (const task of dashboard.tasks) {
      ...
      row.createEl('small', { text:task.due || '기한 미정' });
    ...
    const signals = root.createDiv({ cls:'life-os-card' });
    signals.createEl('h2', { text:'생활 신호' });
    const grid = signals.createDiv({ cls:'life-os-signal-grid' });
    const entries: Array<[string, string]> = [
      ['기한 지난 업무', `${review.overdueOpen}개`],
      ['연락 예정', `${review.duePeople}명`],
      ['활성 목표', `${review.activeGoals}개`],
      ['이번 달 지출', `${review.money.expense.toLocaleString('ko-KR')}원`]
    ];
    for (const [label, value] of entries) {
      const card = grid.createDiv({ cls:'life-os-signal' });
