...
export class PlannerView extends ItemView {
  ...
  async render():Promise<void>{
    ...
    if(plan.focus){focus.createEl('strong',{text:plan.focus.title});focus.createEl('p',{text:`우선순위 ${plan.focus.priority??3} · 예상 ${plan.focus.estimatedMinutes??30}분${plan.focus.due?` · 마감 ${plan.focus.due}`:''}`});focus.createEl('small',{text:`에너지 피크 ${settings.energyPeakStart}–${settings.energyPeakEnd}${peakSlot?` · 빈 슬롯 ${peakSlot.startTime}–${peakSlot.endTime}`:' · 빈 슬롯 없음'}`});if(plan.focus.path){const open=focus.createEl('button',{text:'열기'});open.addEventListener('click',()=>void this.app.workspace.openLinkText(plan.focus?.path??'','',false));}}else focus.createEl('p',{text:'지금 바로 처리해야 할 열린 업무가 없습니다.'});

    const todayCard=root.createDiv({cls:'life-os-card'});todayCard.createEl('h2',{text:'오늘'});todayCard.createEl('p',{text:`계획 ${plan.plannedMinutes}분 · 버퍼 ${plan.bufferMinutes}분 · 재충전 ${settings.rechargeMinutes}분${plan.overload?' · 과부하 주의':''}`});
    ...
    if(conflicts.length){todayCard.createEl('h3',{text:`일정 충돌 ${conflicts.length}건`});for(const {a,b} of conflicts.slice(0,5))todayCard.createEl('p',{text:`⚠ ${a.date} ${a.title} ↔ ${b.title}`});}
    const smallTasks=snapshot.tasks.filter((task)=>task.status!=='done'&&(task.estimatedMinutes??30)<=15).sort((a,b)=>(b.priority??3)-(a.priority??3)).slice(0,3);if(smallTasks.length){todayCard.createEl('h3',{text:'자투리 시간 후보'});for(const task of smallTasks)todayCard.createEl('p',{text:`• ${task.title} · ${task.estimatedMinutes??30}분`});}
    ...
    if(overdueBlocks.length){const recovery=root.createDiv({cls:'life-os-card'});recovery.createEl('h2',{text:'미룬 일정 복구'});recovery.createEl('p',{text:'미룬 일정은 죄책감 목록이 아니라 다시 배치할 작업입니다.'});for(const block of overdueBlocks.slice(0,5)){const row=recovery.createDiv({cls:'life-os-recovery-row'});const copy=row.createDiv();copy.createEl('strong',{text:block.title});copy.createEl('small',{text:`${block.date} · ${block.startTime}–${block.endTime}`});const actions=row.createDiv({cls:'life-os-actions'});if(block.path){for(const [label,action] of [['내일로','tomorrow'],['5분으로','shrink'],['완료','complete']] as const){const button=actions.createEl('button',{text:label});button.addEventListener('click',async()=>{await this.repository.recoverBlock(block.path??'',action);new Notice(`${block.title}: ${label}`);await this.render();});}}}}

    const habitCard=root.createDiv({cls:'life-os-card'});habitCard.createEl('h2',{text:'습관 체크인'});if(!(snapshot.habits??[]).length)habitCard.createEl('p',{text:'lifeos_type: habit 노트를 추가하면 여기에 표시됩니다.'});for(const habit of snapshot.habits??[]){const row=habitCard.createDiv({cls:'life-os-row'});const checkbox=row.createEl('input');checkbox.type='checkbox';checkbox.checked=habit.checkins.includes(today);row.createEl('span',{text:habit.title});checkbox.addEventListener('change',async()=>{if(!habit.path)return;await this.repository.setHabitCheckin(habit.path,today,checkbox.checked);new Notice(checkbox.checked?'습관을 기록했습니다.':'오늘 기록을 취소했습니다.');await this.render();});}

    const reviewCard=root.createDiv({cls:'life-os-card'});reviewCard.createEl('h2',{text:'회고 · Analytics'});reviewCard.createEl('p',{text:`업무 완료 ${review.tasks.completed}/${review.tasks.total} · 기한 지난 업무 ${review.overdueOpen}개 · 활성 프로젝트 ${review.activeProjects}개 · 활성 목표 ${review.activeGoals}개 · 연락 예정 ${review.duePeople}명`});reviewCard.createEl('p',{text:`이번 달 수입 ${review.money.income.toLocaleString('ko-KR')}원 · 지출 ${review.money.expense.toLocaleString('ko-KR')}원`});reviewCard.createEl('p',{text:`오늘 시간 사용 기록 ${review.timeUse.totalMinutes}분`});for(const category of review.timeUse.categories.slice(0,5))reviewCard.createEl('p',{text:`${category.category}: ${category.minutes}분`});for(const habit of review.habits)reviewCard.createEl('p',{text:`${habit.title}: 연속 ${habit.streak}일`});reviewCard.createEl('p',{text:'업무·습관·시간·재정·관계 지표는 하나의 생산성 점수로 합치지 않습니다.'});

    const source=root.createDiv({cls:'life-os-card'});source.createEl('h2',{text:'채택한 시간 관리 원칙'});source.createEl('p',{text:'우선순위, 현실적인 계획, 시간 사용 일지, 버퍼, 방해 최소화, 에너지 피크, 자투리 시간, 80/20, 오늘/내일 구분, 재충전, 완벽주의 방지, 유연한 재계획을 반영합니다.'});const link=source.createEl('a',{text:'참조 근거: 「시간을 절약하는 20가지 방법」'});link.href=TIME_SAVING_SOURCE_URL;link.target='_blank';link.rel='noopener noreferrer';
  }
