...
import type { LifeOsPerson } from '../../core/domain';
...
export class RelationshipsView extends ItemView {
  ...
  async render():Promise<void>{
    ...
    if(!people.length){root.createDiv({cls:'life-os-empty',text:'아직 등록된 관계가 없습니다. 빠른 추가에서 사람을 먼저 등록해 보세요.'});return;}
  ...
  private renderPerson(container:HTMLElement,person:LifeOsPerson,appointments:NonNullable<Awaited<ReturnType<VaultRepository['snapshot']>>['appointments']>,today:string):void{
    ...
    const history=container.createDiv({cls:'life-os-card'});history.createEl('h2',{text:'관계 타임라인'});
    const all=[...upcoming.slice(1),...past].sort((a,b)=>`${b.date} ${b.startTime}`.localeCompare(`${a.date} ${a.startTime}`));if(!all.length)history.createEl('p',{text:'아직 쌓인 만남 기록이 없습니다.'});for(const appointment of all)this.renderAppointment(history,appointment,appointment.status==='planned');
  }

  private renderAppointment(container:HTMLElement,appointment:NonNullable<Awaited<ReturnType<VaultRepository['snapshot']>>['appointments']>[number],planned:boolean):void{
