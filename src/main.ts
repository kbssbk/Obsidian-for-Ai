...
export default class LifeOsPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.addRibbonIcon('plus-circle', 'Life OS 빠른 등록', () => new QuickCaptureModal(this.app, this.repository, () => void this.refreshOpenViews()).open());
    this.addCommand({ id:'quick-capture-life-os', name:'Life OS: 빠른 등록', callback:() => new QuickCaptureModal(this.app, this.repository, () => void this.refreshOpenViews()).open() });
  ...
  onunload(): void {
    for (const type of [DASHBOARD_VIEW_TYPE, PROJECTS_VIEW_TYPE, TIMELINE_VIEW_TYPE, PLANNER_VIEW_TYPE, AREAS_VIEW_TYPE]) this.app.workspace.detachLeavesOfType(type);
  }
