...
export class LifeOsSettingTab extends PluginSettingTab {
  ...
  display():void{
    const {containerEl}=this;containerEl.empty();containerEl.createEl('h2',{text:'라이프 OS 설정'});containerEl.createEl('p',{text:'필요한 기능은 켜고 끌 수 있으며, 계획 규칙은 자신의 생활에 맞게 조정할 수 있습니다.'});
    this.addToggle('오늘 대시보드','오늘 할 일과 생활 신호를 한눈에 봅니다.','dashboardEnabled');
    this.addToggle('프로젝트 진행도','수동 또는 자동 진행률로 프로젝트를 봅니다.','projectsEnabled');
    ...
    containerEl.createEl('h3',{text:'계획 방식'});
    containerEl.createEl('p',{text:'계획을 너무 빽빽하게 잡지 않고, 여유 시간·집중 시간대·재충전 시간을 함께 고려합니다.'});
    ...
    this.addSlider('여유 시간 비율','예상 밖 상황을 위해 비워 둘 비율입니다.',0,50,5,'plannerBufferPercent');
