...
type LifeField=string|number|boolean|string[];
...
function pushYamlField(lines:string[],key:string,value:LifeField):void{if(Array.isArray(value)){lines.push(`${key}:`);for(const item of value)lines.push(`  - ${JSON.stringify(item)}`);return;}lines.push(`${key}: ${yamlScalar(value)}`);}

export class VaultRepository {
  constructor(private readonly app:App){}

  async snapshot():Promise<LifeOsSnapshot>{
    const snapshot:LifeOsSnapshot={blocks:[],tasks:[],projects:[],goals:[],milestones:[],goalActions:[],habits:[],people:[],appointments:[],moneyEntries:[],budgets:[],recurringPayments:[],savingsGoals:[],timeLogs:[]};
    for(const file of this.app.vault.getMarkdownFiles())this.collectFile(file,snapshot);
    return snapshot;
  }

  async createLifeNote(kind:string,title:string,fields:Record<string,LifeField>):Promise<string>{
    const folderMap:Record<string,string>={task:'Inbox',block:'Schedule',goal:'Goals',milestone:'Goals',goal_action:'Goals',habit:'Habits',person:'People',appointment:'Appointments',money:'Money',budget:'Money',recurring_payment:'Money',savings_goal:'Money',time_log:'Time Logs',reflection:'Review'};
    ...
    for(const [key,value] of Object.entries(fields))pushYamlField(lines,key,value);
    lines.push('---','','');await this.app.vault.create(path,lines.join('\n'));return path;
  }

  createInboxTask(title:string,due='',priority=3,estimatedMinutes=30,recurrence:TaskRecurrence='none'):Promise<string>{return this.createLifeNote('task',title,{status:'todo',due,priority:Math.max(1,Math.min(5,Math.round(priority))),estimated_minutes:Math.max(5,Math.round(estimatedMinutes)),research_done:false,draft_done:false,recurrence});}

  createAppointment(title:string,fields:{date:string;startTime:string;endTime:string;location:string;purpose:string;personIds:string[];note:string;nextAction:string}):Promise<string>{return this.createLifeNote('appointment',title,{date:fields.date,start_time:fields.startTime,end_time:fields.endTime,location:fields.location,purpose:fields.purpose,person_ids:fields.personIds,note:fields.note,next_action:fields.nextAction,status:'planned'});}

  async createFocusLog(title:string,taskId:string,startedAt:number,endedAt:number):Promise<string>{const start=new Date(startedAt);const end=new Date(endedAt);const date=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit'}).format(start);const time=(value:Date)=>new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Seoul',hour:'2-digit',minute:'2-digit',hour12:false}).format(value);return this.createLifeNote('time_log',title,{date,start_time:time(start),end_time:time(end),category:'집중',task:taskId});}
  ...
  async setAppointmentStatus(path:string,status:'planned'|'done'|'cancelled'):Promise<void>{await this.updateFrontmatter(path,(fm)=>{fm.status=status;});}
  ...
  private applyTaskCompletion(fm:Record<string,unknown>,done:boolean):void{if(!done){fm.status='todo';return;}const recurrence=(typeof fm.recurrence==='string'?fm.recurrence:'none') as TaskRecurrence;if(recurrence!=='none'){fm.last_completed=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());fm.due=shiftDate(typeof fm.due==='string'?fm.due:'',recurrence);fm.status='todo';fm.research_done=false;fm.draft_done=false;}else fm.status='done';}
