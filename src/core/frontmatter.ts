import type { AppointmentStatus, GoalAction, GoalStatus, HabitFrequency, LifeOsAppointment, LifeOsBlock, LifeOsBudget, LifeOsGoal, LifeOsHabit, LifeOsMilestone, LifeOsMoneyEntry, LifeOsPerson, LifeOsProject, LifeOsRecurringPayment, LifeOsSavingsGoal, LifeOsTask, LifeOsTimeLog, LifeOsStatus, MoneyKind, ProgressMode, ProjectStatus, TaskRecurrence } from './domain';
...
export type ParsedEntity =
  ...
  | { kind:'appointment'; value:LifeOsAppointment }
...
export function parseLifeOsEntity(path:string,basename:string,frontmatter:Frontmatter):ParsedEntity|null{
  const type=text(frontmatter.lifeos_type);const id=text(frontmatter.lifeos_id,path);const title=text(frontmatter.title,basename);
  ...
  if(type==='task')return{kind:'task',value:{id,title,projectId:text(frontmatter.project),due:text(frontmatter.due),status:text(frontmatter.status,'todo') as LifeOsStatus,researchDone:bool(frontmatter.research_done),draftDone:bool(frontmatter.draft_done),priority:Math.max(1,Math.min(5,number(frontmatter.priority,3))),estimatedMinutes:Math.max(5,number(frontmatter.estimated_minutes,30)),recurrence:text(frontmatter.recurrence,'none') as TaskRecurrence,path,...(optional(frontmatter.last_completed)?{lastCompleted:optional(frontmatter.last_completed)}:{})}};
  ...
  if(type==='appointment')return{kind:'appointment',value:{id,title,date:text(frontmatter.date),startTime:text(frontmatter.start_time),endTime:text(frontmatter.end_time),personIds:textArray(frontmatter.person_ids),status:text(frontmatter.status,'planned') as AppointmentStatus,path,...(optional(frontmatter.location)?{location:optional(frontmatter.location)}:{}),...(optional(frontmatter.purpose)?{purpose:optional(frontmatter.purpose)}:{}),...(optional(frontmatter.note)?{note:optional(frontmatter.note)}:{}),...(optional(frontmatter.next_action)?{nextAction:optional(frontmatter.next_action)}:{})}};
  ...
  if(type==='time_log')return{kind:'timeLog',value:{id,title,date:text(frontmatter.date),startTime:text(frontmatter.start_time),endTime:text(frontmatter.end_time),path,...(optional(frontmatter.category)?{category:optional(frontmatter.category)}:{}),...(optional(frontmatter.task)?{taskId:optional(frontmatter.task)}:{})}};
  return null;
}
